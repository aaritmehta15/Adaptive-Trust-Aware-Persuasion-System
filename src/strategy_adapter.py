...
class StrategyAdapter:
    ...
    def select(self, in_recovery: bool) -> str:
        ...
        if in_recovery:
            available = {
                s: self.weights[s] for s in self.RECOVERY_ALLOWED if s in self.weights
            }
        else:
    ...
    def adapt(self, strategy: str, rejection_info: Dict):
        ...
        if rejection_info["trust_concern"]:
            ...
            self.weights["Transparency"] = min(
                1.0, self.weights["Transparency"] * 1.5  # Stronger boost
            )
            # Also boost Empathy slightly
            if "Empathy" in self.weights:
                self.weights["Empathy"] = min(1.0, self.weights["Empathy"] * 1.2)
